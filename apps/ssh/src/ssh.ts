...
const BANNER =
  `${green(LOGO)}\r\n` +
  `Personal developer docs over SSH.\r\n\r\n` +
  `Use ${chalk.dim('ssh docs.sh <command>')} to search and read your docs:\r\n\r\n` +
  `${bg(pad(38))}\r\n` +
  `${bg(`  ${chalk.dim(`# Setup using claude`)}                `)}\r\n` +
  `${bg(`  $ ssh docs.sh setup | claude      `)}\r\n` +
  `${bg(pad(38))}\r\n\r\n` +
  `${bg(pad(43))}\r\n` +
  `${bg(`  ${chalk.dim(`# Or append directly to AGENTS.md`)}        `)}\r\n` +
  `${bg(`  $ ssh docs.sh agents >> AGENTS.md    `)}\r\n` +
  `${bg(pad(43))}\r\n\r\n` +
  `Or explore yourself with grep/find/cat:\r\n\r\n`
  ...
  const server = new Server(
    ...
    (client, info) => {
      ...
      const endSession = (reason: string) => {
        ...
        if (activeChannel) {
          activeChannel.write(
            `\r\n\r\n${green('Session timed out. Reconnect by running: ssh docs.sh')}\r\n\r\n`,
          )
        }

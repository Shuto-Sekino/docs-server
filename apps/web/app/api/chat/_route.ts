...
const SYSTEM_PROMPT = `You are a personal documentation assistant. You help users find and understand their personal developer reference docs.

You have access to a bash tool. To search and read docs, use:
...
Examples:
  ...
  ssh docs.sh grep -r 'docker' /docs/ --include='*.md' -l

All docs live under /docs/ as markdown files. Use standard Unix tools (grep, find, cat, head, etc.) to search and read them.
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    tools: {
      bash: tool({
        description: 'Execute a bash command to search and read personal docs',

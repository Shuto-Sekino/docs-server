...
const EXEC_API_URL = process.env.EXEC_API_URL ?? 'http://localhost:8080/api/exec'
...
const sshCommand = defineCommand('ssh', async (args) => {
  ...
  if (target !== 'docs.sh') {
    return {
      stdout: '',
      stderr: `ssh: only docs.sh is supported as a remote target\n`,
      exitCode: 255,
    }
  }
  ...
  if (rest.length === 0) {
    return {
      stdout: '',
      stderr: `usage: ssh docs.sh <command>\n`,
      exitCode: 1,
    }
  }

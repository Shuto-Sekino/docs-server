...
// ssh command routing
// ---------------------------------------------------------------------------
describe('ssh docs.sh routing', () => {
  it('routes ssh docs.sh <cmd> to EXEC_API_URL and returns stdout', async () => {
    ...
    const result = await executeBashCommand('ssh docs.sh echo hello')
  ...
  it('returns stderr from API when command fails', async () => {
    ...
    const result = await executeBashCommand('ssh docs.sh ls /nonexistent')
  ...
  it('returns error on non-ok HTTP response', async () => {
    ...
    const result = await executeBashCommand('ssh docs.sh broken')
...
describe('ssh missing remote command', () => {
  it('returns usage error when no remote command is given', async () => {
    const result = await executeBashCommand('ssh docs.sh')

...
const TOOLS = [
  { label: 'Claude Code', file: 'CLAUDE.md', command: 'ssh docs.sh setup | claude' },
  { label: 'Codex', file: 'AGENTS.md', command: 'codex "$(ssh docs.sh setup)"' },
  { label: 'Cursor', file: 'AGENTS.md', command: 'agent "$(ssh docs.sh setup)"' },
  { label: 'OpenCode', file: 'AGENTS.md', command: 'opencode --prompt "$(ssh docs.sh setup)"' },
  { label: 'Gemini', file: 'GEMINI.md', command: 'gemini "$(ssh docs.sh setup)"' },
  { label: 'Copilot', file: 'AGENTS.md', command: 'copilot -i "$(ssh docs.sh setup)"' },
] as const

const TRY_COMMAND = 'ssh docs.sh'
...
export function SetupSnippet() {
  ...
  const appendCommand = `ssh docs.sh agents >> ${tool.file}`
  ...
  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col gap-10">
      <div className="flex flex-col items-center gap-4 max-w-lg mx-auto w-full">
        <p className="font-mono text-sm text-[#888]">Browse personal docs over SSH:</p>
